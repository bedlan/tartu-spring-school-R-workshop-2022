...
```{r}
# read_csv("data/uralex/forms.csv") %>%
#   select(lexeme=Value, Language_ID) %>%
#   left_join(read_csv("data/uralex/languages.csv"), by=c("Language_ID" = "ID")) %>%
#   View()


read_tsv("data/lexical-data.tsv") %>% 
  filter(uralex_mng %in% c("fire", "water", "earth")) %>%
  left_join(read_csv("data/uralex/languages.csv"), by=c("language"="Name")) %>%
  select(language, item, uralex_mng, cogn_set, Latitude, Longitude, Subgroup) %>%
  ggplot(aes(x=Longitude, y=Latitude, colour=cogn_set, label=item)) + geom_text() + facet_wrap(uralex_mng~.) +
  theme(legend.position = "none")
```

**NOTE** We really want to plot latitude-longitude data on a map. There are ways you can do this: if you want to do something fancy you can do it in QGIS (after the next course) — you would just need to prepare the data in R, e.g.
```{r message=FALSE}
read_tsv("./data/lexical-data.tsv") %>% 
  filter(uralex_mng == "fire") %>%
  left_join(read_csv("./data/uralex/languages.csv"), by=c("language"="Name")) %>%
  select(language, item, uralex_mng, cogn_set, Latitude, Longitude, Subgroup) %>%
  # this line puts the processed data into a csv file
  write_csv("fire.csv")
```


```{r message=FALSE}
# Install this if you haven't done so already
...
langdata <- read_tsv("./data/lexical-data.tsv") %>% 
  filter(uralex_mng == "earth") %>%
  left_join(read_csv("./data/uralex/languages.csv"), by=c("language"="Name")) %>%
  select(language, item, uralex_mng, cogn_set, Latitude, Longitude, Subgroup)
...
ggplot(data=world) +
  ...
  # scatterplot above)
  geom_text(data=langdata, aes(x=Longitude, y=Latitude, colour=cogn_set, label=item)) +
